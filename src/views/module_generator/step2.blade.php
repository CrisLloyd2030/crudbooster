            ...
            <form method="post" action="{{Route('ModulsControllerPostStep3')}}">
                ...
                <table class="table-display table table-striped">
                    ...
                    @if($cb_col)
                        @foreach($cb_col as $c)
                            <tr>
                                <td><input style="border-radius: 7px;" value='{{$c["label"]}}' type='text' name='column[]' onclick='showColumnSuggest(this)'
                                           onKeyUp='showColumnSuggestLike(this)' placeholder='Column Name' class='column form-control notfocus' value=''/></td>
                                <td><input style="border-radius: 7px;" value='{{$c["name"]}}' type='text' name='name[]' onclick='showNameSuggest(this)' onKeyUp='showNameSuggestLike(this)'
                                           placeholder='Field Name' class='name form-control notfocus' value=''/></td>
                                <td><input style="border-radius: 7px;" value='{{ @explode(",",$c["join"])[0] }}' type='text' name='join_table[]' onclick='showTable(this)'
                                           onKeyUp='showTableLike(this)' placeholder='Table Name' class='join_table form-control notfocus' value=''/></td>
                                <td><input style="border-radius: 7px;" value='{{ @explode(",",$c["join"])[1] }}' type='text' name='join_field[]' onclick='showTableField(this)'
                                           onKeyUp='showTableFieldLike(this)' placeholder='Field Name Shown' class='join_field form-control notfocus' value=''/>
                                </td>
                                <td><input style="border-radius: 7px;" type='text' name='callbackphp[]' class='form-control callbackphp notfocus' value='{{$c["callback_php"]}}'
                                           placeholder="Optional"/></td>
                                <td><input style="border-radius: 7px;" value='{{$c["width"]?:0}}' type='number' name='width[]' class='form-control'/></td>
                                <td>
                                    <select style="border-radius: 7px;" class='form-control is_image' name='is_image[]'>
                                        <option {{ (!$c['image'])?"selected":""}} value='0'>N</option>
                                        <option {{ ($c['image'])?"selected":""}} value='1'>Y</option>
                                    </select>
                                </td>
                                <td>
                                    <select style="border-radius: 7px;" class='form-control is_download' name='is_download[]'>
